...
cv::Mat HBitmapToMat(HBITMAP hBmp)
{
    ...
    bi.biHeight = -bmp.bmHeight;   // flip vertically
    bi.biPlanes = 1;
    bi.biBitCount = 32;
    bi.biCompression = BI_RGB;

    GetDIBits(hMemDC, hBmp, 0, bmp.bmHeight, mat.data, (BITMAPINFO*)&bi, DIB_RGB_COLORS);
...
cv::Mat takeScreenshot_full()
{
    ...
    // Capture ALL monitors + Taskbar + Wallpaper
    BitBlt(hMemoryDC, 0, 0, width, height, hScreen, x, y, SRCCOPY | CAPTUREBLT);
...
void drawGrid(cv::Mat &img)
{
    ...
    {
        line(img, Point(x, 0), Point(x, height), Scalar(0, 0, 0), 1);
        putText(img, std::to_string(x), Point(x + 2, height - 5),
    ...
    {
        line(img, Point(0, y), Point(width, y), Scalar(0, 0, 0), 1);
        putText(img, std::to_string(height - y), Point(2, y - 2),
